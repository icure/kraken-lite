...
object CouchDbAdmin {
	...
	/** Design document ids across every database a kraken-lite instance uses. */
	fun allDesignDocumentIds(): List<String> =
		E2eConfig.expectedDatabases.filter { databaseExists(it) }.flatMap { designDocumentIds(it) }
 ...
 */
suspend fun waitUntil(what: String, timeoutSeconds: Long = 180, condition: () -> Boolean) {
