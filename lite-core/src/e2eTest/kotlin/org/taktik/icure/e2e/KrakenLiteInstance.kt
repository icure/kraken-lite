...
class KrakenLiteInstance private constructor(
	...
	companion object {
		...
		private suspend fun awaitHealthy(process: Process, port: Int, logFile: File, scenario: String) {
...
/**
 * Boots an instance for the duration of [block], destroying it afterward even if the block fails.
 */
suspend fun <T> withKrakenLite(
	scenario: String,
	properties: Map<String, String> = emptyMap(),
	block: suspend (KrakenLiteInstance) -> T,
): T {
	val liteInstance = KrakenLiteInstance.start(scenario, properties)
	liteInstance.use { liteInstance ->
		return block(liteInstance)
	}
}
