...
private abstract class IcureObjectStorageMigrationImpl<T : HasDataAttachments<T>, D>(
	private val dao: D,
...
) : ScheduledIcureObjectStorageMigration<T> where D : GenericDAO<T>, D : AttachmentManagementDAO<T>{
	...
	private suspend fun tryMigration(task: ObjectStorageMigrationTask): Boolean = (
		dao.get(datastoreInstanceProvider.getInstanceAndGroup(), task.entityId)?.let { entity ->
	...
	private suspend fun doMigration(entity: T, key: String, dataAttachment: DataAttachment) {
		...
		val newRev =
			if (entity.attachments?.containsKey(dataAttachment.couchDbAttachmentId) == true)
				dao.deleteAttachment(datastoreInstanceProvider.getInstanceAndGroup(), entity.id, entity.rev!!, dataAttachment.couchDbAttachmentId!!)
			else
				entity.rev!!
	...
	private fun loadAttachment(entity: T, attachmentId: String): Flow<DataBuffer>? = flow {
		runCatching {
			dao.getAttachment(datastoreInstanceProvider.getInstanceAndGroup(), entity.id, attachmentId)
		}.getOrNull()?.map {
			DefaultDataBufferFactory.sharedInstance.wrap(it)
		}?.also {
			emitAll(it)
		}
	}
...
) :	DocumentObjectStorageMigration, ScheduledIcureObjectStorageMigration<Document> by object : IcureObjectStorageMigrationImpl<Document, DocumentDAO>(
	documentDAO,
