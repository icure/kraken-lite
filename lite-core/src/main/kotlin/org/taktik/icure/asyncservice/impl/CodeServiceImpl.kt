...
class CodeServiceImpl(
	...
	override fun create(codes: List<Code>): Flow<Code> = codeLogic.create(codes)
	...
	override suspend fun deleteCode(codeId: String, rev: String): DocIdentifier = codeLogic.deleteEntity(codeId, rev).let { DocIdentifier(it.id, it.rev) }

	override fun deleteCodes(codeIds: List<IdAndRev>): Flow<DocIdentifier> = codeLogic.deleteEntities(codeIds).map { DocIdentifier(it.id, it.rev) }

	override suspend fun undeleteCode(codeId: String, rev: String): Code = codeLogic.undeleteEntity(codeId, rev)

	override fun undeleteCodes(codeIds: List<IdAndRev>): Flow<Code> = codeLogic.undeleteEntities(codeIds)

	override suspend fun purgeCode(codeId: String, rev: String): DocIdentifier = codeLogic.purgeEntity(codeId, rev)

	override fun purgeCodes(codeIds: List<IdAndRev>): Flow<DocIdentifier> = codeLogic.purgeEntities(codeIds)
	...
	): Flow<MergeResult> = codeLogic.solveConflicts(limit, ids, strategy)
}
