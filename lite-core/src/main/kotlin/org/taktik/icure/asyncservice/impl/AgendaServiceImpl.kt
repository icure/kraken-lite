...
class AgendaServiceImpl(
	...
	override fun createAgendas(agendas: List<Agenda>): Flow<Agenda> = agendaLogic.createAgendas(agendas)
	...
	override fun purgeAgendas(ids: List<IdAndRev>): Flow<DocIdentifier> = agendaLogic.purgeEntities(ids)
	override suspend fun undeleteAgenda(id: String, rev: String): Agenda = agendaLogic.undeleteEntity(id, rev)
	override fun undeleteAgendas(ids: List<IdAndRev>): Flow<Agenda> = agendaLogic.undeleteEntities(ids)
	...
	override suspend fun modifyAgenda(agenda: Agenda): Agenda = agendaLogic.modifyAgenda(agenda)
	override fun modifyAgendas(agendas: List<Agenda>): Flow<Agenda> = agendaLogic.modifyEntities(agendas)
	...
	): Flow<MergeResult> = agendaLogic.solveConflicts(limit, ids, strategy)
}