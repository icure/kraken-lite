...
class ContactServiceImpl(
	...
	override fun deleteContacts(ids: List<IdAndRev>): Flow<Contact> = contactLogic.deleteEntities(ids)
	override suspend fun deleteContact(id: String, rev: String?): Contact = contactLogic.deleteEntity(id, rev)
	override suspend fun purgeContact(id: String, rev: String): DocIdentifier = contactLogic.purgeEntity(id, rev)
	override fun purgeContacts(contactIds: List<IdAndRev>): Flow<DocIdentifier> = contactLogic.purgeEntities(contactIds)

	override suspend fun undeleteContact(id: String, rev: String): Contact = contactLogic.undeleteEntity(id, rev)
	override fun undeleteContacts(contactIds: List<IdAndRev>): Flow<Contact> = contactLogic.undeleteEntities(contactIds)

	override suspend fun modifyContact(contact: Contact): Contact = contactLogic.modifyContact(contact)
	...
	override fun modifyContacts(contacts: Collection<Contact>): Flow<Contact> = contactLogic.modifyContacts(contacts.toList())
	...
	): Flow<MergeResult> = contactLogic.solveConflicts(limit, ids, strategy)
}
