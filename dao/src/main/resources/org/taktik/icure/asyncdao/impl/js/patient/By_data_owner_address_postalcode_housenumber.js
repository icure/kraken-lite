map = function (doc) {
	...
	var emit_normalized_substrings = function (k, txt, postalCode, houseNumber, docId, latin_map) {
	...
	var emit_patients_by_address = function (hcparty, doc, latin_map) {
		doc.addresses.forEach(function (address) {
			emit_normalized_substrings(hcparty, ((address.street || '') + '|' + (address.city || '')).replace(new RegExp('\\s', 'g'), '').replace(new RegExp('\\W', 'g'), '').toLowerCase(), address.postalCode, address.houseNumber, doc._id, latin_map);
		});
	};
